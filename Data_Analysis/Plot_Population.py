...
def plotSamplesLN(DataFrame, sampleID, ax, FontSize=None):
...
def plotSamplesMicroEnv(DataFrame, sampleID, ax1, ax2, FontSize=None):
  df = DataFrame.loc[DataFrame['sample'] == sampleID]
  df.loc[:]['simulation_time'] = df['simulation_time']/1440.0 # convert min to day
  ...
  if FontSize:
      for item in ([ax2.title, ax2.xaxis.label, ax2.yaxis.label, ax2.yaxis.offsetText,ax1_2.yaxis.label,ax1_2.yaxis.offsetText] + ax2.get_xticklabels() + ax2.get_yticklabels()+ ax1_2.get_yticklabels()):
          item.set_fontsize(FontSize)
  for text in leg.get_texts():
...
def plot_PopCells(df_output, sampleID,FontSize=None,FigName=None): # Patients examples: NC=2, MC = 93, SC=5, NC+MC=9, NC+SC=7, MC+SC=106, -- (CC and MC and SC)= 6
    fig, (ax1,ax2,ax3) = plt.subplots(1,3,figsize=(16, 4))
    plotSamplesLN(df_output, sampleID,ax2,FontSize=FontSize)
    plotSamplesMicroEnv(df_output, sampleID,ax1,ax3,FontSize=FontSize)
